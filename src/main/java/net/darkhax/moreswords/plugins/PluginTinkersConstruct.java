...
public class PluginTinkersConstruct {
	...
	public static void createMaterial(EnumMoreSwords data) {
		...
		tag.setInteger("Id",
				ConfigurationHandler.initialIDRange + data.ordinal());
		...
		FMLInterModComms.sendRuntimeMessage(Constants.MOD_ID, "TConstruct",
				"addMaterial", tag);
	}
	...
	public static void createPartCastingMaterial(EnumMoreSwords data) {
		...
		Block swordFluidBlock = new BlockSwordFluid(swordFluid, data.swordName)
				.setBlockName("msm." + data.swordName);
		GameRegistry.registerBlock(swordFluidBlock, "msm.fluid."
				+ data.swordName);
		NBTTagCompound tag = new NBTTagCompound();
		tag.setInteger("MaterialId",
				ConfigurationHandler.initialIDRange + data.ordinal());
		tag.setString("FluidName", "msm.fluid." + data.swordName);
		tag.setInteger("Amount", 144);
		FMLInterModComms.sendRuntimeMessage(Constants.MOD_ID, "TConstruct",
				"addPartCastingMaterial", tag);
	}

	public static void createPartBuilderMaterial(EnumMoreSwords data) {

		NBTTagCompound tag = new NBTTagCompound();
		ItemStack swordStack = new ItemStack(
				SwordItems.swordList.get(data.swordName));
	...
	public static class BlockSwordFluid extends BlockFluidClassic {
		...
		public void registerBlockIcons(IIconRegister register) {
			...
			flowingIcon = register.registerIcon("moreswords:fluid_" + fluidName
					+ "_flowing");
