...
public class EnchantmentKeenEdge extends EnchantmentBase {
	...
	protected EnchantmentKeenEdge(int id, int weight, String unlocalizedName,
	...
	public void onEntityHit(AttackEntityEvent event) {
		...
		if (isLiving(event.target)) {
			...
			if (isValidPlayer(event.entityLiving)) {
				...
				event.target.attackEntityFrom(DamageSource.magic,
						(float) (level(stack) + Utils
								.getItemWeaponDamage(stack)));
			}
		}
	}
}