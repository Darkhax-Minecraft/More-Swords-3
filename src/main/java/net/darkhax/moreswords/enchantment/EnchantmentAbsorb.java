...
public class EnchantmentAbsorb extends EnchantmentBase {
	...
	protected EnchantmentAbsorb(int id, int weight, String unlocalizedName,
	...
	public void onEntityHit(AttackEntityEvent event) {
		...
		if (d < cfg.absorbChance) {
			...
			if (isValidPlayer(event.entityPlayer)) {
				...
				int food = Constants.RND
						.nextIntII(cfg.absorbMin, cfg.absorbMax);
				...
				if (event.entityPlayer.worldObj.isRemote)
					event.entityPlayer.worldObj
							.playSoundAtEntity(event.entityPlayer,
									"random.burp", 0.5F,
									event.entityPlayer.worldObj.rand
											.nextFloat() * 0.1F + 0.9F);
			}
		}
	}
