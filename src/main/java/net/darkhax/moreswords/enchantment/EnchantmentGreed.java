...
public class EnchantmentGreed extends EnchantmentBase {
	...
	protected EnchantmentGreed(int id, int weight, String unlocalizedName,
			...
			if (isValidPlayer(event.entityLiving)) {
				...
				if (Math.random() < (cfg.greedChance * level(stack))) {
					...
					if (!living.worldObj.isRemote) {
						...
						living.worldObj.spawnEntityInWorld(new EntityXPOrb(
								living.worldObj, living.posX, living.posY,
								living.posZ, rnd.nextIntII(cfg.greedMin,
										cfg.greedMax)));
					}
				}
			}
