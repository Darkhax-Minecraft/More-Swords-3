...
public class EnchantmentVitality extends EnchantmentBase {
	...
	protected EnchantmentVitality(int id, int weight, String unlocalizedName,
	 ...
	 * enchantment level, gives a 4 heart health boost for 3 seconds multiplied
	 * by the enchantment level and a low level instant health boost for 0.05
	 * seconds.
	 */
	@SubscribeEvent
		...
		if ((event.action.equals(PlayerInteractEvent.Action.RIGHT_CLICK_AIR))) {
			...
			if (isValidPlayer(event.entityPlayer)) {
				...
				player.addPotionEffect(new PotionEffect(Potion.regeneration.id,
						cfg.vitalityRegenTime * enchLevel,
						cfg.vitalityRegenLevel));
				player.addPotionEffect(new PotionEffect(
						Potion.field_76434_w.id, cfg.vitalityHeartsTime
								* enchLevel, cfg.vitalityHeartsLevel));
				player.addPotionEffect(new PotionEffect(Potion.heal.id,
						cfg.vitalityHealTime, cfg.vitalityHealLevel));
