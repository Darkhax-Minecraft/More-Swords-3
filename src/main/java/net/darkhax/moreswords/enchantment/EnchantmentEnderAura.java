...
public class EnchantmentEnderAura extends EnchantmentBase {
	...
	protected EnchantmentEnderAura(int id, int weight, String unlocalizedName,
	...
	public void attemptWarp(EntityPlayer living) {
		...
		Entity target = (Entity) living.worldObj.loadedEntityList
				.get(Constants.RND.nextIntII(1,
						living.worldObj.loadedEntityList.size() - 1));

		if (isLiving(target)) {

			if (Utils.isEntityWithinRange(living, target, cfg.enderAuraRange)) {

				living.setPositionAndUpdate(target.posX, target.posY,
						target.posZ);
				return;
			}
		}
