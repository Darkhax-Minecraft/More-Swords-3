...
public class EnchantmentFrostWave extends EnchantmentBase {
	...
	protected EnchantmentFrostWave(int id, int weight, String unlocalizedName,
					...
					if (living.worldObj.loadedEntityList.get(i) != living) {
						...
						if (living.worldObj.loadedEntityList.get(i) instanceof EntityLiving) {
							...
							if (Utils
									.isEntityWithinRange(
											living,
											(EntityLiving) living.worldObj.loadedEntityList
													.get(i), cfg.frostRange)) {

								EntityLiving closeEntity = (EntityLiving) living.worldObj.loadedEntityList
										.get(i);
								closeEntity.addPotionEffect(new PotionEffect(
										Potion.moveSlowdown.id, cfg.frostTime,
										cfg.frostLevel));
							}
						}
					}
