...
public class EnchantmentFrozenAspect extends EnchantmentBase {
	...
	protected EnchantmentFrozenAspect(int id, int weight,
			String unlocalizedName, int minLevel, int maxLevel, Item item) {
		...
		if (isLiving(event.target)) {
			...
			if (isValidPlayer(event.entityLiving)) {
				...
				((EntityLiving) event.target).addPotionEffect(new PotionEffect(
						Potion.moveSlowdown.id, cfg.frozenTime * level(stack),
						cfg.frozenLevel * level(stack)));
			}
		}
	}
}